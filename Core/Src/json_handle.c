...
void data_to_json(int16_t data, UINT tag);
...
void data_to_json_string(const char* str, UINT tag) ;
...
void json_handle_thread_entry(ULONG thread_input)
{
    ...
    while(1) {
        ...
        while (tx_queue_receive(&sensor_data_queue, &sensor_data, TX_NO_WAIT) == TX_SUCCESS) {
            data_to_json(sensor_data.value, sensor_data.tag);
            data_count++;
        }
        // char* timestamp = get_timestamp();
        // data_to_json_string(timestamp,TIMESTAMP_DATA);
        if (data_count > 0) {
...
// void data_to_json_string(const char* str, UINT tag) {
//   if (tag == 7) { 
//       cJSON_AddItemToArray(timestamp_array, cJSON_CreateString(str));
//   }
// }
void free_json_string(char* json_string) {
  if (json_string != NULL) {
